...
function loadConfig() {
  ...
  if (!fs.existsSync(file)) {
    throw new Error("\nFile 'bare.config.json' not found.\nRun 'bare init' to create the config file.\n");
  }
...
function runSSH(server, cmd, description) {
  ...
  if (options.verbose) {
    log("info", `    target: ${server.user}@${server.host}`);
    log("info", `    action: ${description || "running command"}`);
    log("info", `    command: ${cmd}`);
  }
...
function scpTo(server, localFile, remotePath) {
  ...
  if (options.verbose) {
    log("info", `    from: ${localFile}`);
    log("info", `    to: ${server.user}@${server.host}:${remotePath}`);
  }
...
function copyWellKnown(server, sourceDir, targetDir) {
  ...
  try {
    ...
    log("warn", `The .well-known copy failed, continuing anyway: ${err.message}`);
  }
}
...
async function deploy() {
  ...
  const deployToServer = async (server) => {
    ...
    // Run pre-scripts locally for this server
      ...
      try {
        ...
        if (previousReleaseForRollback && previousReleaseForRollback.trim() !== "") {
          ...
          const prevExists = runSSH(
            ...
            "Verifying if previous release exists...",
          );
          ...
          if (prevExists !== "yes") {
            ...
            if (rollbackIndex > 0) {
              ...
              runSSH(
                ...
                "Updating previous to version before the rollback target...",
              );
            }

            // Re-run start-script for rollback
            if (server.startScript) {
              try {
                runSSH(
                  server,
            ...
            if (config.healthCheck?.url) {
              try {
                ...
                log("error", "Previous release health check failed after rollback!");
              }
            } else {
  ...
  config.servers.forEach((server) => {
    try {
      if (options.dryRun) {
        ...
        log("info", "20260220123456-v1.0.1");
        log("info", "20260220150000-v1.0.2  (previous)");
        log("info", "20260220162341-v1.0.3  (current)");
...
function init() {
  ...
  if (fs.existsSync(configPath)) {
    throw new Error("\nFile 'bare.config.json' already exists in current directory!\n");
  }
  ...
  if (!fs.existsSync(pkgPath)) {
    ...
    log("info", "Creating file 'package.json' with version 0.1.0...\n");

    const defaultPkg = { version: "0.1.0" };

    fs.writeFileSync(pkgPath, JSON.stringify(defaultPkg, null, 2));
    log("success", `File 'package.json' created with { "version": "0.1.0" }.`);
  }
  ...
  if (!fs.existsSync(gitignorePath)) {
    ...
    log("success", "File '.gitignore' created with 'bare.config.json' entry.");
...
async function checkForUpdate() {
  try {
    ...
    if (isNewerVersion(currentVersion, latestVersion)) {
      log("warn", `\nA new version of bare-deploy is available: ${latestVersion} (you're on ${currentVersion})`);
...
(async () => {
  if (options.queryVersion) {
    ...
    log("info", `Bare Deploy v${getCliVersion()}`);
    return;
  }
